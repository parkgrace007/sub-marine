...
async function add8h12hAlerts() {
  console.log('\n📊 Adding test alerts for 8h and 12h timeframes...\n')

  const timeframes = ['8h', '12h']
  ...
  for (const timeframe of timeframes) {
    for (const symbol of symbols) {
      ...
      // Add 4-6 alerts per combination
      const numAlerts = 4 + Math.floor(Math.random() * 3)
      ...
      for (let i = 0; i < numAlerts; i++) {
        ...
        const rng = Math.floor(20 + Math.random() * 60)
        const variations = [
          `${baseMessage} (${rng})`,
  ...
  console.log('\n' + '='.repeat(60))
  console.log(`✅ Successfully inserted ${totalInserted} alerts for 8h and 12h timeframes!`)
